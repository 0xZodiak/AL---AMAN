...
export default function AgentDashboard() {
  const { leads, loading, error, globalDateFrom, globalDateTo } = useData();
  const { currentUser } = useAuth();

  const myLeads = useMemo(() => {
    if (!leads) return [];
    let filtered = leads.filter(l => l.agent_id === currentUser.id);
    if (globalDateFrom) filtered = filtered.filter(l => l.added_date >= globalDateFrom);
    if (globalDateTo)   filtered = filtered.filter(l => l.added_date <= globalDateTo);
    return filtered;
  }, [leads, currentUser.id, globalDateFrom, globalDateTo]);

  if (loading) {
    return (
      <div className="agent-dash-loading">
  ...
  if (error) {
    return (
      <div className="agent-dash-error">
        <p>⚠️ حدث خطأ: {error}</p>
      </div>
    );
  }
  ...
  const pct        = Math.round((bookings / TARGET) * 1000) / 10;
  const commission = bookings > TARGET ? (bookings - TARGET) * 20 : 0;
  const revenue    = myLeads.filter(l => l.status === 'مؤكد').reduce((sum, l) => sum + (Number(l.booking_price) || 0), 0);
      ...
      <div className="agent-main-card" style={{ borderColor: `${color}33` }}>
        ...
        <div className="main-stats">
          <div className="main-stat-row">
            ...
            <div className="main-stat" style={{ borderColor: 'rgba(251,191,36,0.3)', background: 'rgba(251,191,36,0.05)' }}>
              <span className="ms-val" style={{ color: '#fbbf24' }}>{revenue.toLocaleString('ar-EG')} <small style={{fontSize:12}}>ريال</small></span>
              <span className="ms-lbl">💰 قيمة الحجوزات</span>
            </div>
          </div>
      ...
      <div className="commission-section">
        <div className="comm-card" style={commission > 0
          ...
          <div className="comm-body">
            ...
            {commission > 0 ? (
              <>
                <div className="comm-amount">{commission} <span>جنيه</span></div>
                <div className="comm-formula">({bookings} - {TARGET}) × 20 جنيه</div>
              </>
            ) : (
              <>
                <div className="comm-amount" style={{ color: 'rgba(255,255,255,0.3)', fontSize: 20 }}>0 جنيه</div>
              ...
              return (
                <div key={lead.id} className="recent-item">
                  ...
                  <div className="recent-info">
                    ...
                    <div className="recent-meta">{lead.phone} · {lead.added_date}</div>
                  </div>
