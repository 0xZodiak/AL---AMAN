...
export default function Agents() {
  const { users, loading, error, getAgentStats, getRanking } = useData();
  const { currentUser } = useAuth();

  const ranking = useMemo(() => {
    if (loading || !users) return [];
    return getRanking();
  }, [users, getRanking, loading]);

  if (loading) {
    return (
      <div className="agents-loading">
        <span className="spinner" />
        <p>جاري تحميل ترتيب الإيجنت...</p>
      </div>
    );
  }
  ...
  const visibleRanking = currentUser.role === 'team_leader'
    ? ranking.filter(a => a.team_leader_id === currentUser.id).map((a, i) => ({ ...a, rank: i + 1 }))
    : ranking;
        ...
        <div className="podium-section">
          ...
          <div className="podium">
            ...
            <div className="podium-item second">
              ...
              <div className="podium-bonus">+{BONUSES[1]} جنيه</div>
            ...
            <div className="podium-item first">
              ...
              <div className="podium-bonus">+{BONUSES[0]} جنيه</div>
            ...
            <div className="podium-item third">
              ...
              <div className="podium-bonus">+{BONUSES[2]} جنيه</div>
            ...
            <div key={agent.id} className={`agent-card ${isTop3 ? 'top3' : ''}`} style={isTop3 ? { borderColor: `${rankColor}44` } : {}}>
              ...
              <div className="agent-card-header">
                ...
                <div className="agent-info">
                  ...
                  <div className="agent-team" style={{ color: 'rgba(255,255,255,0.4)' }}>
                    التيم: {users.find(u => u.id === agent.team_leader_id)?.name || '—'}
                  </div>
                </div>
              </div>
              ...
              <div className="agent-stats-grid">
                ...
                <div className="agent-stat">
                  ...
                  <span className="stat-lbl">كوميشن ج</span>
                </div>
              </div>
              ...
              {agent.rank <= 3 && (
                <div className="agent-bonus" style={{ borderColor: `${rankColor}44`, background: `${rankColor}10` }}>
                  🎁 مكافأة الترتيب: <strong style={{ color: rankColor }}>{BONUSES[agent.rank - 1]} جنيه</strong>
                </div>
              )}
              ...
              {stats.commission > 0 && (
                <div className="commission-badge">
                  💰 كوميشن إضافي: <strong>{stats.commission} جنيه</strong>
                </div>
              )}
            </div>
          );
            ...
            <tbody>
              {visibleRanking.map(agent => (
                <tr key={agent.id}>
                  ...
                  <td>{users.find(u => u.id === agent.team_leader_id)?.name?.split(' ')[0] || '—'}</td>
                  <td>{agent.total}</td>
                  ...
                  <td className="commission-cell">{agent.commission > 0 ? `${agent.commission} ج` : '—'}</td>
                  <td className="bonus-cell">{agent.rank <= 3 ? `${BONUSES[agent.rank - 1]} ج` : '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
